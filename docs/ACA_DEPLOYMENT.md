...
This guide provides step-by-step instructions for deploying the Admin Service to **Azure Container Apps** with built-in Dapr support.
...
# Create topic for admin events (for audit service consumption)
...
The local `.dapr/components/event-bus.yaml` is configured for RabbitMQ. For Azure Container Apps, create an Azure Service Bus component:
...
> - Admin Service is a **publisher-only** service (does not consume events)
...
# Get URLs for downstream services (must be deployed first)
USER_SERVICE_URL=$(az containerapp show \
  --name user-service \
  --resource-group $RESOURCE_GROUP \
  --query properties.configuration.ingress.fqdn \
  --output tsv 2>/dev/null || echo "user-service.internal")

ORDER_SERVICE_URL=$(az containerapp show \
  --name order-service \
  --resource-group $RESOURCE_GROUP \
  --query properties.configuration.ingress.fqdn \
  --output tsv 2>/dev/null || echo "order-service.internal")

PRODUCT_SERVICE_URL=$(az containerapp show \
  --name product-service \
  --resource-group $RESOURCE_GROUP \
  --query properties.configuration.ingress.fqdn \
  --output tsv 2>/dev/null || echo "product-service.internal")
...
az containerapp create \
  ...
  --env-vars \
    ...
    "PRODUCT_SERVICE_URL=https://$PRODUCT_SERVICE_URL" \
    "LOG_LEVEL=info"
```

> **Note**: Admin Service has no database - it proxies requests to User Service, Order Service, and Product Service. Ensure those services are deployed first.
...
az keyvault secret set --vault-name $KV_NAME --name "xshopai-jwt-public-key" --value "<jwt-public-key>"
...
```bash
# Delete container app (safe - only removes admin-service)
