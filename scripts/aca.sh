...
ACR_LOGIN_SERVER=$(az acr show --name "$ACR_NAME" --query loginServer -o tsv 2>/dev/null) || { print_error "ACR not found"; exit 1; }
...
[ -n "$IDENTITY_ID" ] && print_success "Managed Identity: $MANAGED_IDENTITY" || print_warning "Managed Identity not found"
...
# Map environment
APP_ENV="development"
[ "$ENVIRONMENT" = "prod" ] && APP_ENV="production"
...
# Service tokens (admin-service calls many downstream services)
SVC_AUTH_TOKEN=$(az keyvault secret show --vault-name "$KEY_VAULT" --name "service-auth-token" --query "value" -o tsv 2>/dev/null || echo "")
SVC_AUDIT_TOKEN=$(az keyvault secret show --vault-name "$KEY_VAULT" --name "service-audit-token" --query "value" -o tsv 2>/dev/null || echo "")
...
SVC_NOTIFICATION_TOKEN=$(az keyvault secret show --vault-name "$KEY_VAULT" --name "service-notification-token" --query "value" -o tsv 2>/dev/null || echo "")
...
ENV_VARS=(
    ...
    "NAME=$SERVICE_NAME"
    "NODE_ENV=$APP_ENV"
    "NOTIFICATION_SERVICE_APP_ID=notification-service"
    ...
    "SERVICE_AUDIT_TOKEN=$SVC_AUDIT_TOKEN"
    "SERVICE_AUTH_TOKEN=$SVC_AUTH_TOKEN"
    "SERVICE_INVENTORY_TOKEN=$SVC_INVENTORY_TOKEN"
    "SERVICE_NAME=$SERVICE_NAME"
    "SERVICE_NOTIFICATION_TOKEN=$SVC_NOTIFICATION_TOKEN"
